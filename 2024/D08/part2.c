...
int main() {
  ...
  for(uint64_t i = 0; i < size(poss); i++) {
    ...
    for(uint64_t j = 0; j < size(poss); j++) {
      ...
      for(int64_t x_new_1 = x_act + x_dist, y_new_1 = y_act + y_dist;
          valid(rows, cols, x_new_1, y_new_1); x_new_1 += x_dist, y_new_1 += y_dist) {
        char grid_ = *((char*) get(grid, pos_vector(cols, x_new_1, y_new_1)));
        int64_t one = 1;
        set(antinodes, pos_vector(cols, x_new_1, y_new_1), &one);
      }

      for(int64_t x_new_2 = x_act - x_dist, y_new_2 = y_act - y_dist;
          valid(rows, cols, x_new_2, y_new_2); x_new_2 += x_dist, y_new_2 += y_dist) {
