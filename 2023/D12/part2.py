...
for condition, group in conditions:
    backup = condition
    grbackup = copy.deepcopy(group)
    for i in range(4):
        condition = condition + "?" + backup
        group.extend(grbackup)
    conditions2.append([condition, group])
print(sum([count(condition, group) for condition, group in conditions2]))
