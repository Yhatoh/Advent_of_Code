...
get_valid_coord coord_sym l board =
  (unique_coords [coord_digit | coord_digit <- l, (check_adyacent coord_sym coord_digit)] board [])

all_valid_coords [] l board = []
all_valid_coords (h:t) l board = 
  if (length (get_valid_coord h l board)) == 2
    then (get_valid_coord h l board) ++ (all_valid_coords t l board)
    else (all_valid_coords t l board)

multiply_two [] = []
multiply_two (x:y:t) = [x * y] ++ (multiply_two t)

main = do
  cont <- getContents
  let cont_split = (init (splitOn "\n" cont))
  let res = sum (multiply_two [read (get_num_by_coord x cont_split) :: Int | x <- (all_valid_coords (get_coords_symbol 0 cont_split) (get_coords 0 cont_split)) cont_split])
  print res
